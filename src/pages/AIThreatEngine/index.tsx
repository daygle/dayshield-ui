...
function formatRelativeFromUnix(unixSeconds: number | null): string {
  ...
  const unit =
    absMs >= day ? 'day' : absMs >= hour ? 'hour' : absMs >= minute ? 'minute' : 'second';
...
function riskBadge(score: number) {
  ...
  const barColor = score >= 0.9 ? 'bg-red-500' : score >= 0.7 ? 'bg-amber-500' : 'bg-green-500';
...
function AIThreatEngineContent() {
  ...
  const formatUnixDateTime = useCallback(
  ...
  const aiValidation = useMemo(() => {
    ...
    const errors = {
      automatic_blocking:
        !aiForm.enabled && aiForm.automatic_blocking
  ...
  const handleToggleSuricataEnabled = async () => {
    ...
    try {
      ...
      setSourceError(null);
      addToast(
        `Suricata ${res.data.enabled ? 'enabled' : 'disabled'} for AI correlation sources.`,
        'success'
      );
    } catch (err) {
  ...
  const handleToggleCrowdSecEnabled = async () => {
    ...
    try {
      ...
      setSourceError(null);
      addToast(
        `CrowdSec ${res.data.enabled ? 'enabled' : 'disabled'} for AI correlation sources.`,
        'success'
      );
    } catch (err) {
  ...
  const handleFeedback = useCallback(
    async (id: string, feedback: 'false_positive' | 'confirmed_malicious') => {
  ...
  const threatColumns: Column<ThreatRow>[] = useMemo(
    () => [
      ...
      },
      {
        key: 'src_ip',
        header: 'Src IP',
        render: (row) => <span className="font-mono">{String(row.src_ip)}</span>,
      },
      {
        key: 'dst_ip',
        header: 'Dst IP',
        render: (row) => <span className="font-mono">{String(row.dst_ip)}</span>,
      },
      {
        key: 'protocol',
        header: 'Protocol',
        render: (row) => <span className="uppercase">{String(row.protocol)}</span>,
      },
      {
        key: 'risk_score',
        header: 'Risk Score',
        render: (row) => riskBadge(Number(row.risk_score)),
      },
      {
        key: 'blocked',
        header: 'Blocked',
        render: (row) =>
          yesNoBadge(Boolean(row.blocked), 'Blocked', 'No', 'bg-red-100 text-red-700'),
      },
      {
        key: 'escalated',
        header: 'Escalated',
        render: (row) =>
          yesNoBadge(Boolean(row.escalated), 'Escalated', 'No', 'bg-amber-100 text-amber-700'),
      },
      {
        key: 'quarantine',
        header: 'Quarantine',
        render: (row) =>
          yesNoBadge(Boolean(row.quarantine), 'Quarantine', 'No', 'bg-purple-100 text-purple-700'),
      },
      {
        key: 'actions',
        header: 'Actions',
    ...
    () => [
      {
        key: 'ip',
        header: 'IP',
        render: (row) => <span className="font-mono">{String(row.ip)}</span>,
      },
      {
        key: 'added_at',
        header: 'Added',
    ...
    <div className="space-y-6">
        ...
        {selectedThreat && (
          <div className="space-y-4 text-sm">
            <dl className="grid grid-cols-2 gap-x-6 gap-y-2">
              <Detail
                label="ID"
                value={<span className="font-mono break-all">{selectedThreat.id}</span>}
              />
              <Detail label="Timestamp" value={formatUnixDateTime(selectedThreat.timestamp)} />
              <Detail
                label="Source IP"
                value={<span className="font-mono">{selectedThreat.src_ip}</span>}
              />
              <Detail
                label="Destination IP"
                value={<span className="font-mono">{selectedThreat.dst_ip}</span>}
              />
              <Detail
                label="Source Port"
                value={selectedThreat.src_port === null ? '-' : String(selectedThreat.src_port)}
              />
              <Detail
                label="Destination Port"
                value={selectedThreat.dst_port === null ? '-' : String(selectedThreat.dst_port)}
              />
              ...
              <Detail
                ...
                value={
                  selectedThreat.alert_severity == null
                    ? '-'
                    : String(selectedThreat.alert_severity)
                }
              />
              <Detail
                label="Model Label"
                value={selectedThreat.label == null ? '-' : String(selectedThreat.label)}
              />
              <Detail
                label="Risk Score"
                value={`${Math.round(selectedThreat.risk_score * 100)}%`}
              />
              <Detail label="Blocked" value={selectedThreat.blocked ? 'Yes' : 'No'} />
              <Detail
                label="Block Expires"
                value={
                  selectedThreat.block_expires_at === null
                    ? 'Permanent'
                    : formatUnixDateTime(selectedThreat.block_expires_at)
                }
              />
              ...
              <Detail
                label="Manually Unblocked"
                value={selectedThreat.manually_unblocked ? 'Yes' : 'No'}
              />
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <SummaryMetric
          ...
          helper={
            aiSettings.automatic_blocking
              ? 'High-risk sources can be blocked'
      ...
      <Card
        title="AI Threat Engine Settings"
        subtitle="Tune local scoring, blocking, and feedback learning policy"
      >
        {aiLoading ? (
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              <div className="space-y-3 lg:col-span-2">
                ...
                <ToggleRow
                  ...
                  onChange={(checked) =>
                    setAiForm((prev) => ({ ...prev, automatic_blocking: checked }))
                  }
                />
                {aiValidation.automatic_blocking && (
                ...
                <ToggleRow
                  ...
                  onChange={(checked) =>
                    setAiForm((prev) => ({ ...prev, training_enabled: checked }))
                  }
                />
              </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <FormField
                ...
                onChange={(e) => {
                  ...
                  setAiForm((prev) => ({
                    ...prev,
                    escalation_window_seconds: Number.isFinite(parsed)
                      ? parsed
                      : prev.escalation_window_seconds,
                  }));
                }}
              />
              <FormField
                ...
                onChange={(e) => {
                  ...
                  setAiForm((prev) => ({
                    ...prev,
                    block_duration_seconds: Number.isFinite(parsed)
                      ? parsed
                      : prev.block_duration_seconds,
                  }));
                }}
              />
              <FormField
                ...
                onChange={(e) => {
                  ...
                  setAiForm((prev) => ({
                    ...prev,
                    model_learning_rate: Number.isFinite(parsed)
                      ? parsed
                      : prev.model_learning_rate,
                  }));
                }}
              />
            </div>
      ...
      <Card
        ...
        actions={
          <Button
            size="sm"
            variant="secondary"
            onClick={loadCorrelationSources}
      ...
      >
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm px-6 py-5">
                ...
                <p
                  className={`text-base font-semibold ${suricataConfig?.enabled ? 'text-green-600' : 'text-gray-500'}`}
                >
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm px-6 py-5">
                ...
                <p
                  className={`text-base font-semibold ${crowdSecConfig?.enabled ? 'text-green-600' : 'text-gray-500'}`}
                >
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow-sm px-6 py-5">
                ...
                <p className="text-base font-semibold text-gray-900">
                  {crowdSecConfig ? `${crowdSecConfig.update_interval}s` : '-'}
                </p>
              </div>
            </div>
              ...
              <div className="rounded-lg border border-gray-200 bg-white p-5">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600">
                      Primary IDS/IPS source for AI threat scoring.
                    </p>
                  </div>
                  <Link
                    to="/suricata"
                  ...
                  >
                    ...
                    >
                      <path
                        ...
                        d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4m-1-9h3m0 0v3m0-3L10 15"
                      />
                    </svg>
                  </Link>
                </div>
                <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                  <div>
                    ...
                    <dd
                      className={`font-semibold ${suricataConfig?.enabled ? 'text-green-700' : 'text-gray-500'}`}
                    >
                      {suricataConfig?.enabled ? 'Enabled' : 'Disabled'}
                    </dd>
                  </div>
                  <div>
                    ...
                    <dd className="font-semibold uppercase text-gray-900">
                      {suricataConfig?.mode ?? '-'}
                    </dd>
                  </div>
                  <div className="col-span-2">
                    ...
                    <dd className="font-semibold text-gray-900">
                      {suricataConfig?.interfaces.length ?? 0}
                    </dd>
                  </div>
                </dl>
              ...
              <div className="rounded-lg border border-gray-200 bg-white p-5">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-600">
                      External reputation decisions used as AI context signals.
                    </p>
                  </div>
                  <Link
                    to="/crowdsec"
                ...
                <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
                  <div>
                    <dt className="text-gray-500">Status</dt>
                    <dd
                      className={`font-semibold ${crowdSecConfig?.enabled ? 'text-green-700' : 'text-gray-500'}`}
                    >
                      {crowdSecConfig?.enabled ? 'Enabled' : 'Disabled'}
                    </dd>
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <dd className="font-semibold text-gray-900 break-all">
                      {crowdSecConfig?.lapi_url || 'Not configured'}
                    </dd>
                  </div>
                </dl>
                <div className="mt-4 flex flex-wrap gap-2">
        ...
        <p className="mt-3 text-xs text-gray-500">
          Suricata and CrowdSec still have their full dedicated settings pages. This section is for
          AI source correlation controls and quick operational toggles.
        </p>
      </Card>
            ...
            <div className="mb-1 flex justify-center">
              ...
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M12 3l8 3v5c0 5-3.5 9.5-8 11-4.5-1.5-8-6-8-11V6l8-3z"
                />
              </svg>
            </div>
