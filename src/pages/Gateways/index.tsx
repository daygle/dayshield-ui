...
export default function Gateways() {
  ...
  const columns: Column<GatewayRow>[] = [
    ...
    },
    {
      key: 'interface',
      header: 'Interface',
      render: (row) => interfaceLabel(row.interface as string),
    },
    {
      key: 'active_ip',
      header: 'Gateway IP',
    ...
    },
    {
      ...
      header: 'Monitor IP',
      render: (row) =>
        (row.monitor_ip as string) || <span className="text-gray-400 text-xs">-</span>,
    },
    {
      key: 'state',
      header: 'Health',
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="space-y-4">
          ...
          {isEditing && String(form.name).endsWith('_AUTO') && (
            ...
            >
              <input className="input" value={editingActiveIp ?? '-'} readOnly />
            </FormField>
          )}
          <FormField
            label="Monitor IP"
          ...
          <FormField
            label="Weight"
            hint="1–100, lower = higher priority. Used for multi-WAN groups."
          >
            <input
              type="number"
              min={1}
              ...
              value={form.weight}
              onChange={(e) =>
                setForm((f) => ({ ...f, weight: parseInt(e.target.value, 10) || 1 }))
              }
            />
          </FormField>
      ...
      <Card
        title="Gateways"
        subtitle="Configure upstream gateways, priorities, and health checks"
        actions={
          <button
            onClick={openAdd}
            className="btn-icon btn-icon-secondary"
        ...
        <div className="space-y-4">
          {defaultIface && (
            <p className="text-sm text-gray-500 mt-0.5">
              Default route via:{' '}
              <span className="font-medium text-gray-700">{interfaceLabel(defaultIface)}</span>
            </p>
          )}
