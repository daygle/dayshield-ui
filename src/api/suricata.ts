...
export const getSuricataConfig = (): Promise<ApiResponse<SuricataConfig>> =>
  apiClient.get<ApiResponse<SuricataConfig>>('/suricata/config').then((r) => r.data);
...
export const createSuricataRuleset = (data: {
  name: string;
  url?: string;
  path?: string;
  enabled?: boolean;
}): Promise<ApiResponse<SuricataRuleset>> =>
  apiClient.post<ApiResponse<SuricataRuleset>>('/suricata/rulesets', data).then((r) => r.data);
...
export const checkSuricataRulesetUpdates = (): Promise<ApiResponse<SuricataRuleset[]>> =>
  apiClient.post<ApiResponse<SuricataRuleset[]>>('/rulesets/check-all-updates').then((r) => r.data);
...
export const installSuricataRuleset = (
  id: string | number
): Promise<ApiResponse<SuricataRuleset>> => postRulesetAction(id, 'install');

export const checkSuricataRulesetUpdate = (
  id: string | number
): Promise<ApiResponse<SuricataRuleset>> => postRulesetAction(id, 'check-update');

export const updateManagedSuricataRuleset = (
  id: string | number
...
export const updateSuricataRulesetDisabledRules = (
  ...
  apiClient
    ...
    >(`/rulesets/${encodeRulesetId(id)}/disabled-rules`, { ids: disabledIds })
    .then((r) => r.data);
...
export const getInterfaceSuricataConfig = (
  ...
  apiClient
    .get<
      ApiResponse<InterfaceSuricataConfig>
...
export const updateInterfaceSuricataConfig = (
  ...
  apiClient
    .post<
      ApiResponse<InterfaceSuricataConfig>
    >(`/interfaces/${encodeURIComponent(interfaceName)}/suricata`, { monitored })
    .then((r) => r.data);
