...
export const listBackups = (): Promise<ApiResponse<BackupEntry[]>> =>
  apiClient.get<ApiResponse<Record<string, unknown>[]>>('/backup/list').then((r) => ({
...
export const createBackup = (req: CreateBackupRequest): Promise<ApiResponse<BackupEntry>> =>
  apiClient.post<ApiResponse<Record<string, unknown>>>('/backup/create', req).then((r) => ({
    success: r.data.success,
...
export const restoreBackup = (req: RestoreBackupRequest): Promise<ApiResponse<void>> =>
  apiClient.post<ApiResponse<void>>('/backup/restore', req).then((r) => r.data);
...
export const getBackupSchedule = (): Promise<ApiResponse<BackupSchedule>> =>
  apiClient.get<ApiResponse<BackupSchedule>>('/backup/scheduler').then((r) => r.data);

export const updateBackupSchedule = (
  schedule: BackupSchedule
): Promise<ApiResponse<BackupSchedule>> =>
  apiClient.post<ApiResponse<BackupSchedule>>('/backup/scheduler', schedule).then((r) => r.data);
