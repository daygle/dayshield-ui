...
function buildInterfaceInventory(payload: InterfacesPayload): InterfacesInventory {
  ...
  const configuredEnriched = configured.map((iface) => {
    ...
    const firstIpv4Cidr = kernelIface?.addresses?.find(
      (addr) => addr.includes('.') && addr.includes('/')
    );
    const [kernelIpv4Address, kernelIpv4Prefix] = firstIpv4Cidr
      ? firstIpv4Cidr.split('/')
      : [undefined, undefined];
    const firstIpv6Cidr = kernelIface?.addresses?.find(
      (addr) => addr.includes(':') && addr.includes('/')
    );
    const [kernelIpv6Address, kernelIpv6Prefix] = firstIpv6Cidr
      ? firstIpv6Cidr.split('/')
      : [undefined, undefined];
  ...
  const names = Array.from(new Set([...configuredNames, ...kernelNames])).sort((a, b) =>
    a.localeCompare(b)
  );
...
export const getInterfaces = (): Promise<ApiResponse<NetworkInterface[]>> =>
  apiClient.get<ApiResponse<InterfacesPayload>>('/interfaces').then((r) => ({
    ...r.data,
