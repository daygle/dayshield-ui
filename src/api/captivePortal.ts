...
export const getCaptivePortalConfig = (): Promise<ApiResponse<CaptivePortalConfig>> =>
  apiClient
    .get<ApiResponse<CaptivePortalConfig>>('/captive-portal/config')
    .then((r) => r.data)
...
export const getCaptivePortalStatus = (): Promise<ApiResponse<CaptivePortalStatus>> =>
  apiClient
    .get<ApiResponse<CaptivePortalStatus>>('/captive-portal/status')
    .then((r) => r.data)
...
export const revokeCaptivePortalSession = (sessionId: string): Promise<ApiResponse<void>> =>
  apiClient
    .delete<ApiResponse<void>>(`/captive-portal/sessions/${sessionId}`)
    .then((r) => r.data)
