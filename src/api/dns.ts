...
export const getDnsConfig = (): Promise<ApiResponse<DnsConfig>> =>
  apiClient.get<ApiResponse<DnsConfig>>('/dns/config').then((r) => r.data);
...
export const getDnsOverrides = (): Promise<ApiResponse<DnsOverridesData>> =>
  apiClient.get<ApiResponse<DnsOverridesData>>('/dns/overrides').then((r) => r.data);

export const createDnsOverride = (override: {
  kind: 'host' | 'domain';
  name: string;
  target: string;
...
export const createInterfaceDnsBlocklist = (
  ...
  apiClient
    .post<
      ApiResponse<DnsBlocklistEntry>
    >(`/interfaces/${encodeURIComponent(interfaceName)}/dns/blocklists`, payload)
    .then((r) => r.data);
